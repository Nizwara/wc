...
input_akun() {
    ...
    # Menulis konfigurasi ke file akun.txt dengan warna hijau sebelum tanda '=' dan warna kuning setelah tanda '='
    cat <<EOL > "$AKUN_FILE"
${GREEN}AUTH_EMAIL${NC}=${YELLOW}"$AUTH_EMAIL"${NC}
${GREEN}AUTH_KEY${NC}=${YELLOW}"$AUTH_KEY"${NC}
${GREEN}ACCOUNT_ID${NC}=${YELLOW}"$ACCOUNT_ID"${NC}
${GREEN}YOUR_NAME${NC}=${YELLOW}"$YOUR_NAME"${NC}
${GREEN}ZONE_ID${NC}=${YELLOW}"$ZONE_ID"${NC}
EOL
